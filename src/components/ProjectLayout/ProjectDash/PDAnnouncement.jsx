...
import { usePostAnnounecementMutation } from "../../../redux/api/projectsApi";
...
const PDAnnouncement = () => {
  const [announcements, setAnnouncements] = useState([
    ...
    },
    {
      ...
      author: "Md Asik",
      content: "Another announcement content here.",
      date: "3/27/2025",
      time: "11:30 AM",
    },
    {
      ...
      author: "Md Asik",
      content:
        "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Facere reprehenderit adipisci dolore tempore necessitatibus quo ea saepe vel quia doloremque blanditiis iusto voluptates neque, officiis magni maxime perspiciatis cum odio, non voluptas.",
      date: "3/28/2025",
      time: "1:00 PM",
    },
  ]);
  const [announcementsIndex, setAnnouncementsIndex] = useState(
  ...
  const [postAnnounecement] = usePostAnnounecementMutation();
  ...
  const handleCreateAnnouncement = async () => {
    const response = await postAnnounecement(inputInfo);
  ...
  return (
    <div className="bg-gray-200 rounded-lg py-3 ps-3 pe-2 flex-grow max-w-[460px]">
      <div className="flex justify-between items-start">
        ...
        <button
          onClick={() => setOpenAddAnnModal(!openAddAnnModal)}
        ...
        <Modal isOpen={openAddAnnModal} setIsOpen={setOpenAddAnnModal}>
          <div className="p-3">
            ...
            <textarea
              onChange={(e) =>
                setInputInfo({ ...inputInfo, content: e.target.value })
              }
              className="p-3 text-sm border border-gray-300 w-full rounded-lg mb-4"
              type="text"
      ...
      <div className="nav flex justify-between items-center mt-3">
        ...
        <div className="bg-gray-300 flex gap-3 p-3 rounded-lg">
          <span className="flex items-center gap-2 text-sm font-[500]">
            ...
            {announcements[announcementsIndex].date}
          </span>
          <span className="flex items-center gap-2 text-sm font-[500]">
            <IoMdTime className="text-lg" />
            {announcements[announcementsIndex].time}
          </span>
        </div>
        <button
      ...
      <div className="relative mt-4">
        <div className="bg-white h-[205px] p-3 rounded-lg relative">
          ...
          <h2 className="text-xl font-[500] -mb-1">
            {announcements[announcementsIndex].title.length > 35
              ? announcements[announcementsIndex].title.slice(0, 35) + "..."
              : announcements[announcementsIndex].title}
          </h2>
          <span className="text-sm text-gray-500">
            By {announcements[announcementsIndex].author}
          </span>
          <p className="text-black">
            {announcements[announcementsIndex].content.length > 250
              ? announcements[announcementsIndex].content.slice(0, 220) + "..."
              : announcements[announcementsIndex].content}
          </p>
          <button
            title="Expand"
      ...
      <Modal isOpen={isModalOpen} setIsOpen={setIsModalOpen}>
        <div className="bg-gray-300 p-4">
          ...
          <p className="text-sm text-gray-500">
            By {announcements[announcementsIndex].author} |{" "}
            {announcements[announcementsIndex].date} at{" "}
            {announcements[announcementsIndex].time}
          </p>
          <div
            className={
              announcements[announcementsIndex].content.length > 200
                ? "bg-white p-3 rounded-lg mt-3 h-[400px] overflow-y-auto"
                : "bg-white p-3 rounded-lg mt-3"
            }
          >
            <p className="text-black">
              {announcements[announcementsIndex].content}
