...
import { setLoading, setUserData, uploadImageToImgbb } from "../../redux/features/userSlice";
import toast from "react-hot-toast";
import { useGetUserQuery, useUploadProfilePictureMutation } from "../../redux/api/userApi";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await dispatch(
        uploadImageToImgbb(imageData.selectedFile)
      );
      ...
      if (uploadImageToImgbb.fulfilled.match(response)) {
        ...
        const responseFromBackend = await uploadProfilePicture({
          _id: userData?._id,
          data: uploadedURL,
        });
        ...
        <div className="flex flex-col md:h-[80vh] h-[86vh]">
          <div className="md:m-10 mt-4 flex-grow">
                ...
                <div className="bg-gray-200 hover:bg-gray-300 duration-500 cursor-pointer border-[4px] border-gray-400 border-dashed relative w-3/4 h-3/4 rounded-full overflow-hidden">
                  {imageData?.previewURL ? (
                    <div className="w-full h-full relative">
                      ...
                      />
                      <div
                        className={`bg-[#0000006b] duration-300 ${
                          hover ? "opacity-100" : "opacity-0"
                        } w-full h-full flex items-center justify-center absolute top-0 left-0`}
                      >
                        <h1
                          className={`text-2xl duration-700 uppercase ${
                            hover ? "translate-y-0" : "translate-y-[300px]"
                          } text-white`}
                        >
                          Change
                        </h1>
            ...
            <button
              ...
              className="bg-[#1a1a1a] px-6 py-2 text-white rounded-lg border-[#1a1a1a] border hover:bg-white hover:text-black duration-500 cursor-pointer mt-8"
            >
              {isLoading ? (
